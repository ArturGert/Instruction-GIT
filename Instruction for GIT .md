...
```
Для отмены слияния веток при конфликте можновоспользоваться параметром merge с флагом --abort.
```
$ git merge --abort
```
При конфликте слияния можно использовать параметр reset, чтобы восстановить конфликтующие файлы до стабильного состояния. 
```
$ git reset
```
